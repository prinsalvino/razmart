...
if (isset($_SESSION["uid"])) {
    ...
    $filesError = $_FILES['fileToUpload']['error'];
    ...
    if(in_array($fileActualExt,$allowed)){
        if($filesError ===  0){
            if($_FILES['fileToUpload']['size'] < 1000000){            
                $fileNameNew = $order_id.".".$fileActualExt;
	...
	`city`, `state`, `zip`, `cardname`,`cardnumber`,`expdate`,`prod_count`,`total_amt`,`cvv`, `buktifoto_name`) 
	VALUES ('$order_id', '$user_id','$f_name','$email', 
    '$address', '$city', '$state', '$zip','$cardname','$cardnumberstr','$expdate','$total_count','$prod_total','$cvv', '$fileNameNew')";
